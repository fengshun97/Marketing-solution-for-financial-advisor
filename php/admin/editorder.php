...
if (!isset($_SESSION['admin_login'])) {
	...
	if (isset($_REQUEST['eoid'])) {
		...
		$order_list= "SELECT * FROM orders WHERE id='$eoid'";
		$getposts5 = mysqli_query($con,$order_list) or die(mysqli_error());
	...
	$order_list1="SELECT * FROM admin WHERE id='$user'";
	$result = mysqli_query($con,$order_list1);
	$get_user_email = mysqli_fetch_assoc($result);
		$uname_db = $get_user_email['firstName'];

	$order_list2="SELECT * FROM orders WHERE id='$eoid'";
	$result1 = mysqli_query($con,$order_list2);
		$get_order_info = mysqli_fetch_assoc($result1);
			...
			$order_list3="SELECT * FROM user WHERE id='$eouid'";
			$result2 = mysqli_query($con,$order_list3);
...
$order_list4="SELECT * FROM products WHERE id ='$eopid'";
$getposts = mysqli_query($con,$order_list4) or die(mysqli_error());
	...
	try {
		if(empty($_POST['dstatus'])) {
			throw new Exception('Status can not be empty');
				...
				$order_list5="UPDATE orders SET dstatus='$eodstatus', ddate='$ddate', quantity='$dquantity' WHERE id='$eoid'";
				if(mysqli_query($con,$order_list5)){
					//success message
				header('location: orders.php?eoid='.$eoid.'');
				$success_message = '
...
//triming name
	$order_list6="DELETE FROM orders WHERE id='$eoid'";
	if(mysqli_query($con,$order_list6)){
		...
		<div style="float: left;">
			<div>
				<?php
						...
						<ul style="float: left;">
							<li style="float: left; padding: 0px 25px 25px 25px;">
								<div class="home-prodlist-img">
									<img src="../image/product/'.$item.'/video-icon.png" class="home-prodlist-imgi">
					...
					';}
					elseif ($item=="documents"){echo '
						<ul style="float: left;">
							<li style="float: left; padding: 0px 25px 25px 25px;">
								<div class="home-prodlist-img">
									<img src="../image/product/'.$item.'/pdf-icon.png" class="home-prodlist-imgi">
