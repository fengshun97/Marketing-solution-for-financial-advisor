		...
		if(empty($_POST['email'])) {
			...
			if(isset($opass) && isset($npass) && isset($npass1) && ($opass != "" && $npass != "" && $npass1 != "")){
				if( md5($opass) == $upass){
					if($npass == $npass1){
						$npass = md5($npass);
$orders_list2 = "UPDATE user SET password='$npass' WHERE id='$user'";						
mysqli_query($con,$orders_list2);
						$success_message = '
...
$orders_list3 = "UPDATE user SET password='$npass' WHERE id='$user'";
			if($uemail_db != $email) {
				if(mysqli_query($con,$orders_list3)){
					//success message
					$success_message = '
					<div class="signupform_text" style="font-size: 18px; text-align: center;">
					<font face="bookman">
